...
//  AllInOneInspections
//
//  Created by Chawtech on 16/06/21.
//
...
class ServerClass: NSObject {
    ...
    func getRequestWithUrlParameters(_ sendJson:[String:Any], path:String, successBlock:@escaping (_ response: JSON )->Void , errorBlock: @escaping (_ error: NSError) -> Void ){
        ...
        let toekn = UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN)
        print("token \(toekn)")
        if UserDefaults.standard.object(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) != nil  {
            headerField = ["Content-Type":"application/json", "X-Authorization":X_API_KEY, "token":UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) as! String]
        }
        else {
            headerField = ["Content-Type":"application/json","X-Authorization":X_API_KEY]
        }
    ...
    func sendMultipartRequestToServer(apiUrlStr:String, imageKeyName:String, imageUrl:URL?, successBlock:@escaping (_ response: JSON)->Void , errorBlock: @escaping (_ error: NSError) -> Void ){
        let headerField = ["X-Authorization":X_API_KEY]
    ...
    func sendImageMultipartRequestToServerWithToken(apiUrlStr:String, imageKeyName:String, imageUrl:URL?, successBlock:@escaping (_ response: JSON)->Void , errorBlock: @escaping (_ error: NSError) -> Void ){
        let headerField = ["X-Authorization":X_API_KEY,"APPTOKEN":UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) as! String]
        Alamofire.upload(multipartFormData: { multipartFormData in
            multipartFormData.append(imageUrl! , withName: imageKeyName)
        },to:apiUrlStr, method: .post, headers : headerField,
          encodingCompletion: { encodingResult in
    ...
    func sendMultipartRequestToServerPP(apiUrlStr:String, imageKeyName:String, imageUrl:URL?, successBlock:@escaping (_ response: JSON)->Void , errorBlock: @escaping (_ error: NSError) -> Void ){
        ...
        if UserDefaults.standard.object(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) != nil  {
           ...
           // headerField = ["Content-Type":"application/json","X-Authorization":X_API_KEY]
            headerField = ["X-Authorization":X_API_KEY]
        }
        Alamofire.upload(multipartFormData: { multipartFormData in
            multipartFormData.append(imageUrl! , withName: imageKeyName)
        },to:apiUrlStr, method: .post, headers : headerField,
    ...
    func sendMultipartRequestToServer(urlString:String,fileName:String, sendJson:[String:Any], imageUrl:URL?, successBlock:@escaping (_ response: JSON)->Void , errorBlock: @escaping (_ error: NSError) -> Void) {
       ...
       /* if UserDefaults.standard.bool(forKey: USER_DEFAULTS_KEYS.IS_LOGIN) == true {
            headerField = ["content-type": "application/json","X-Authorization": X_API_KEY, "APPTOKEN": appLoginToken ?? " "]
        }
        else {
            headerField  = ["content-type": "application/json","X-Authorization": X_API_KEY]
        }*/
    ...
    func sendMultipartRequestToServerWithMultipleImages(urlString:String, imageKeyName:String, imageUrl:[URL?], successBlock:@escaping ( _ response: JSON)->Void , errorBlock: @escaping ( _ error: NSError) -> Void ){
           var headerField : [String : String] = [:]
           if UserDefaults.standard.bool(forKey: USER_DEFAULTS_KEYS.IS_LOGIN) == true {
               headerField = ["content-type": "application/json","X-Authorization": X_API_KEY, "TOKEN": appLoginToken ?? " "]
           }
           else {
               headerField = ["content-type": "application/json","X-Authorization": X_API_KEY]
           }
    ...
    func sendMultipartRequestWithMultileFiles(urlString:String,fileNames:[String], _ sendJson:[String:Any], imageUrl:Data, successBlock:@escaping (_ response: JSON)->Void , errorBlock: @escaping (_ error: NSError) -> Void )
    {
        var headerField : HTTPHeaders = [:]
      
        if UserDefaults.standard.object(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) != nil  {
            headerField = ["Content-Type": "multipart/form-data", "X-Authorization":X_API_KEY, "token":UserDefaults.standard.value(forKey: USER_DEFAULTS_KEYS.LOGIN_TOKEN) as! String]
        }
        else {
            headerField = ["Content-Type":"application/json","X-Authorization":X_API_KEY]
        }
...
//                }
          ...
          //  let data=NSData(contentsOf: imageURL)
            let img = UIImage(data:imageUrl)
           // for i in 0..<imageUrl.count{
            let imageData1 = img?.jpegData(compressionQuality: 0.5)
            multipartFormData.append(imageData1!, withName: "files", fileName: "photo" + String(2) + ".jpg", mimeType: "image/jpeg")
                  //  }
